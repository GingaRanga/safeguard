        ...
        PIXELDIMA.CONTACT = function() {
            ...
            var checkout = function() {
                ...
                checkout.validate({
                    //Callback when the form is valid.
                    submitHandler: function() {
                        $("#checkout").addClass('loading-form')
                            // Ajax Submit
                        $.ajax({
                            type: "POST",
                            url: url,
                            datatype: "json",
                            data: {
                                "name": $("#checkout [name=firstname]").val(),
                                "lastname": $("#checkout [name=lastname]").val(),
                                "address": $("#checkout [name=address]").val(),
                                "town": $("#checkout [name=town]").val(),
                                "state": $("#checkout [name=state]").val(),
                                "postcode": $("#checkout [name=postcode]").val(),
                                "email": $("#checkout [name=email]").val(),
                                "phone": $("#checkout [name=phone]").val(),
                            },
                            success: function(data) {
                                if (data === "success") {
                                    $("#contactSuccess").removeClass("hide").addClass("show animated bounceIn");
